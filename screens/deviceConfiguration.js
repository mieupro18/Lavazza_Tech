...
import {SERVER_URL, TOKEN} from '../Utilities/serverip';
...
class DeviceInfo extends Component {
  ...
  fetchDeviceData = async () => {
    this.setState({isLoading: true, deviceData: null});
    fetch(SERVER_URL + '/techapp/deviceInfo', {
      headers: {
        tokenId: TOKEN,
      },
      signal: (await this.getTimeoutSignal(5000)).signal,
    })
      ...
      .then(async resultData => {
        if (resultData.status === 'Success') {
          ...
          if (resultData.infoText === 'config error') {
            ToastAndroid.showWithGravityAndOffset(
              'Reboot Failed. Please provision the device configurations completely',
              ToastAndroid.LONG,
              ToastAndroid.CENTER,
              25,
              50,
          ...
          {this.state.isLoading === true ? (
            ...
            <View style={styles.errorContainer}>
              ...
              <TouchableHighlight
                ...
                <Text style={styles.tryAgainButtonTextStyle}>Try Again</Text>
              </TouchableHighlight>
            </View>
          )}
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  pickerStyle: {textAlign: 'center', color: '#100A45'},
  cancelButtonStyle: {
    justifyContent: 'space-around',
    width: '40%',
    marginBottom: 30,
